# hard-forked from https://github.com/commaai/openpilot/tree/05b37552f3a38f914af41f44ccc7c633ad152a15/selfdrive/car/isotp_parallel_query.py
import time
    ...
    while True:
      ...
      for tx_addr, msg in msgs.items():
        try:
          ...
          print("Error processing UDS response")
        ...
        if response_valid:
          ...
          request_done[tx_addr] = True
          print(f"iso-tp query bad response: 0x{dat.hex()}")

      cur_time = time.monotonic()
      if cur_time - last_response_time > timeout:
        for tx_addr in msgs:
          if (request_counter[tx_addr] > 0) and (not request_done[tx_addr]):
            print(f"iso-tp query timeout after receiving response: {tx_addr}")
        break

      if cur_time - start_time > total_timeout:
        print("iso-tp query timeout while receiving data")
        break
