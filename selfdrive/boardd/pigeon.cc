...
void PandaPigeon::set_baud(int baud) {
  panda->handle->control_write(0xe2, 1, 0);
  panda->handle->control_write(0xe4, 1, baud/300);
}
...
void PandaPigeon::send(const std::string &s) {
  ...
  for (int i=0; i<len; i+=0x20) {
    ...
    panda->handle->bulk_write(2, a, ll+1);
  }
}
...
std::string PandaPigeon::receive() {
  ...
  while (r.length() < 0x1000) {
    int len = panda->handle->control_read(0xe0, 1, 0, dat, sizeof(dat));
    if (len <= 0) break;
...
void PandaPigeon::set_power(bool power) {
  panda->handle->control_write(0xd9, power, 0);
}
